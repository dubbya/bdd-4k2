...
var setupFolders = function (callback) {
    ...
    if (!fs.existsSync(featuredir)) {
        ...
        const localstepdef = path.resolve(featuredir + '/support/step_defs.js')
        const bddstepdef = path.resolve(defaultfeaturesdir + '/support/step_defs.js')
        
        if (fs.existsSync(path.resolve(featuredir + '/support'))) {
            if (fs.existsSync(localstepdef)) {
                const bddstats = fs.statSync(bddstepdef)
                const projstats = fs.statSync(localstepdef)
                
                if (bddstats.mtime != projstats.mtime) {
                    const renamedate = new Date().toISOString().slice(0,10)
                    
                    fs.renameSync(localstepdef, path.resolve(featuredir + '/support/step_defs_' + renamedate + '.js.txt'))
                    fse.copySync(bddstepdef, localstepdef)
                }
            } else {
                fse.copySync(bddstepdef, localstepdef)
            }
